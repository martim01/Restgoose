        ...
        **/
        class RG_EXPORT HttpClient
        {
            public:
                ...
                *   @return <i>clientResponse</i> a clientResponse object containing the HTTP repsonse code and any data sent from the server
                **/
                void Run(std::function<void(const clientResponse&, unsigned int )> pCallback, unsigned int nRunId, const std::chrono::milliseconds& connectionTimeout = std::chrono::milliseconds(5000), const std::chrono::milliseconds& processTimeout = std::chrono::milliseconds(0));
