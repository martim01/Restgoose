...
class RG_EXPORT RestGoose
{
    public:
        ...
        *   @param func std::function that defines the callback function.The function is passed the query data, postData (for a PUT,PATCH or POST) the methodpoint and the userName if any.
        The function should return a response which will be sent back to the client
        *   @return <i>bool</i> true on success
        **/
        bool AddEndpoint(const methodpoint& theEndpoint, std::function<response(const query&, const postData&, const endpoint&, const userName&)> func);
