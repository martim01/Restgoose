...
{
    pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "create_part_data " << contentType;
...
{
    pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "EventWebsocketOpen";
}
...
bool MongooseServer::AuthenticateWebsocket(const subscriber& sub, const Json::Value& jsData)
{
    ...
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "AuthenticateWebsocket";
    if(m_tokenCallback)
    {
    ...
    if(jsData.isMember("user") && jsData["user"].isString() && jsData.isMember("password") && jsData["password"].isString())
    {
        if(auto itUser = m_mUsers.find(userName(jsData["user"].asString()));itUser != m_mUsers.end() && itUser->second.Get() == jsData["password"].asString())
        {
            if(auto itEndpoint = m_mWebsocketAuthenticationEndpoints.find(sub.theEndpoint); itEndpoint != m_mWebsocketAuthenticationEndpoints.end())
            {
                ...
                {
                    pml::log::log(pml::log::Level::kInfo, kLogPrefix) << "Websocket subscriber: " << sub.peer << " authorized";

                    return true;
                }
                else
                {
                    pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket subscriber: " << sub.peer << " not authorized";
                    return false;
                }
            }
            else
            {
                pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket subscriber: " << sub.peer << " endpoint: " << sub.theEndpoint << " has not authorization function";
                return false;
            }
        }
        else
        {
            pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket subscriber: " << sub.peer << " User not found or password not correct";
            return false;
        }
    }
    else
    {
        pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket subscriber: " << sub.peer << " No user or password sent";
        return false;
    }
}
...
bool MongooseServer::AuthenticateWebsocketBearer(const subscriber& sub, const Json::Value& jsData)
{
    ...
    {
        pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket subscriber: " << sub.peer << " No bearer token sent";
        return false;
    }
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "AuthenticateWebsocketBearer token=" << sToken;
...
{
    pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "RestGoose::Server\tEventWebsocketMessage";
    ...
    if(auto itSubscriber = m_mSubscribers.find(pConnection); itSubscriber != m_mSubscribers.end())
    {
        ...
        {
            pml::log::log(pml::log::Level::kError, kLogPrefix) << "Unable to convert '" << sMessage << "' to JSON: " << e.what();
        }
    }
...
void MongooseServer::DoWebsocketAuthentication(mg_connection* pConnection, subscriber& sub, const Json::Value& jsData)
{
    ...
    {
        pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "RestGoose::Server\tHandleInternalWebsocketMessage: Authenticated";
...
void MongooseServer::HandleInternalWebsocketMessage(mg_connection* pConnection, subscriber& sub, const Json::Value& jsData)
{
    ...
    else
    {
        ...
        {
            pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket subscriber: " << sub.peer << " attempted to send data before authenticating. " << " Close the connection";
            m_mSubscribers.erase(pConnection);
            pConnection->is_closing = 1;
        }
    }
...
{
    pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "RestGoose::Server\tHandleExternalWebsocketMessage";
    if(sub.bAuthenticated)
    {
        ...
        {
            pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "" << sub.peer << " has no message methodpoint!";
        }
    }
    else
    {
...
{
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "tWebsocket subscriber: " << sub.peer << " adding subscriptions " << jsData;

    if(jsData["endpoints"].isArray())
    {
        for(const auto& jsEndpoint : jsData["endpoints"])
        {
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "tWebsocket subscriber: " << sub.peer << " adding subscription " << jsEndpoint.asString();
            sub.setEndpoints.emplace(jsEndpoint.asString());
        }
    }
    else
    {
        pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "tWebsocket subscriber: Incorrect JSON";
    }
}

void MongooseServer::RemoveWebsocketSubscriptions(subscriber& sub, const Json::Value& jsData) const
{
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "Websocket subscriber: " << sub.peer << " removing subscriptions " << jsData;
        ...
        case WEBSOCKET_OP_TEXT:
            {
                ...
                pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "Websocket ctl: [" << (int)pMessage->flags << "] " << sData;
            }
            break;
        case WEBSOCKET_OP_CLOSE:
            {
                pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "WebsocketCtl - close";
...
authorised MongooseServer::CheckAuthorizationBasic(mg_http_message* pMessage)
{
    ...
    {
        pml::log::log(pml::log::Level::kInfo, kLogPrefix) << "CheckAuthorization: user '" << sUser <<" with given password not found";
...
methodpoint MongooseServer::GetMethodPoint(mg_http_message* pMessage) const
{
    ...
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "GetMethodPoint: " << sMethod << "\t" << sUri;
...
void MongooseServer::EventHttp(mg_connection *pConnection, int, void* pData)
{
    ...
    else if(InApiTree(thePoint.second))
    {
        ...
        pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "'" << thePoint.second << "' in Api tree";
    ...
    {
        pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "'" << thePoint.second << "' not in Api tree";
...
{
    pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "RestGoose::Server\tEventHttpWebsocket";
    ...
    {   //if we have not set up any users then we are not using authentication so we don't need to authenticate the websocket
        ...
        pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "RestGoose::Server\tEventHttpWebsocket: Authenticated";
    }
...
void MongooseServer::HandleEvent(mg_connection *pConnection, int nEvent, void* pData)
{
    switch (nEvent)
    {
        case MG_EV_ERROR:
            {
                pml::log::log(pml::log::Level::kError, kLogPrefix) << reinterpret_cast<char*>(pData);
            }
            break;
        case MG_EV_OPEN:
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleOpen";
            HandleOpen(pConnection);
            break;
        case MG_EV_ACCEPT:
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleAccept";
            HandleAccept(pConnection);
            break;
        case MG_EV_WS_OPEN:
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleWebsocketOpen";
            EventWebsocketOpen(pConnection, nEvent, pData);
            break;
        case MG_EV_WS_CTL:
            pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "HandleWebsocketCtl";
            EventWebsocketCtl(pConnection, nEvent, pData);
            break;
        case MG_EV_WS_MSG:
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleWebsocketMsg";
            EventWebsocketMessage(pConnection, nEvent, pData);
            break;
        case MG_EV_HTTP_MSG:
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleHTTPMsg";
            pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "HTTP_MSG: " << pConnection;
        ...
        case MG_EV_WRITE:
            ...
            break;
        case MG_EV_CLOSE:
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleClose";
            ...
            break;
        case MG_EV_POLL:    //this gets called each loop
...
void MongooseServer::HandleOpen(mg_connection* pConnection)
{
    if(pConnection)
    {
        ...
        {
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleOpen: Not allowed due to ACL";
            pConnection->is_closing = 1;
        }
        else if(get_number_of_connections(m_mgr) > m_nMaxConnections)
        {
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "HandleOpen: REACHED MAXIMUM";
            pConnection->is_closing = 1;
        }
    }
}
...
void MongooseServer::HandleAccept(mg_connection* pConnection) const
{
    ...
    {
        pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "Accept connection: Turn on TLS";
        struct mg_tls_opts tls_opts;
        if(m_sCa.empty())
        {
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "TLS No CA";
        ...
        {
            pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "TLS We have a CA";
            tls_opts.ca = mg_str(m_sCa.c_str());
        }
        pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "TLS Hostname: " << m_sHostname;
        ...
        {
            pml::log::log(pml::log::Level::kError, kLogPrefix) << "Could not implement TLS";
        }
    }
}
...
bool MongooseServer::Init(const std::filesystem::path& ca, const std::filesystem::path& cert, const std::filesystem::path& key, const ipAddress& addr, unsigned short nPort, const endpoint& apiRoot, bool bEnableWebsocket, bool bSendPings)
{
    ...
    pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "Websockets=" << m_bWebsocket;
...
void MongooseServer::Loop()
{
    ...
    if(m_pConnection)
    {
        ...
        pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "Started: " << m_sServerName;
    ...
    {
        pml::log::log(pml::log::Level::kError, kLogPrefix) << "Could not start webserver";
    }
...
void MongooseServer::DoReplyText(mg_connection* pConnection, const response& theResponse) const
{
    ...
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "DoReply " << theResponse.nHttpCode << "\t" << sReply;
...
{
    pml::log::log(pml::log::Level::kDebug, kLogPrefix) << "DoReplyFile " << theResponse.nHttpCode << "\t" << theResponse.data;
...
void MongooseServer::SendWSQueue()
{
    ...
    if(m_pConnection)
    {
        ...
        while(m_qWsMessages.try_dequeue(message))
        {
            ...
            pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "SendWSQueue: " << sMessage;
            ...
            for (auto pConnection = m_pConnection->mgr->conns; pConnection != nullptr; pConnection = pConnection->next)
            {
                if(is_websocket(pConnection))
                {
                    ...
                    if(itSubscriber != m_mSubscribers.end())
                    {
                        ...
                        {
                            pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "" << itSubscriber->second.theEndpoint << " not yet authenticated...";
                        }
                    }
                }
            }
...
bool MongooseServer::WebsocketSubscribedToEndpoint(const subscriber& sub, const endpoint& anEndpoint) const
{
    ...
    {
        pml::log::log(pml::log::Level::kTrace, kLogPrefix) << "WebsocketSubscribedToEndpoint: " << endp;
            ...
            for (mg_connection* pConnection = m_pConnection->mgr->conns; pConnection != nullptr; pConnection = pConnection->next)
            {
                if(is_websocket(pConnection))
                {
                    ...
                    if(itSub != m_mSubscribers.end())
                    {
                        ...
                        {
                            pml::log::log(pml::log::Level::kWarning, kLogPrefix) << "Websocket from " << itSub->second.peer << " has not responded to PING. Close";
...
{
    pml::log::log(pml::log::Level::kInfo, kLogPrefix) << "MongooseServer::SetStaticDirectory " << sDir;
