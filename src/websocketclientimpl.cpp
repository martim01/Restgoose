...
void WebSocketClientImpl::Loop()
{
    ...
    pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tloop exited";
}
...
bool WebSocketClientImpl::CheckPingPong(std::map<endpoint, connection>::iterator& itConnection, const std::chrono::milliseconds& elapsed)
{
    ...
    if(m_bPingPong&& itConnection->second.bConnected && elapsed.count() > 4000)
    {
        ...
        {
            pml::log::log(pml::log::Level::kWarning, "pml::restgoose") << "WebsocketClient\tWebsocket has not responded to PING. Close " << itConnection->second.bPonged;
            itConnection->second.pConnection->is_closing = 1;
...
void WebSocketClientImpl::Callback(mg_connection* pConnection, int nEvent, void * pEventData)
{
    ...
    switch(nEvent)
    {
        ...
        case MG_EV_OPEN:
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWebsocket connection created" << " connection=" << pConnection->id << "\t" << buffer;           
            break;
        case MG_EV_RESOLVE:
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWebsocket hostname resolved" << " connection=" << pConnection->id << "\t" << buffer;
            break;
        case MG_EV_ACCEPT:
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWebsocket connection accepted" << " connection=" << pConnection->id << "\t" << buffer;
            break;
        case MG_EV_HTTP_MSG:
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWebsocket http message!";
            break;
        case MG_EV_CONNECT:
        ...
        case MG_EV_ERROR:
            ...
            pml::log::log(pml::log::Level::kError, "pml::restgoose") << "WebsocketClient\tWebsocket error: "  << " connection=" << pConnection->id << "\t" << std::string((char*)pEventData) << " errno says " << errno << "\t" << buffer;

        
            MarkConnectionConnected(pConnection, false, errno);
            break;
        case MG_EV_WS_OPEN:
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWebsocket connected " << get_number_of_connections(m_mgr);
    ...
    if(m_pConnectCallback)
        {
            ...
            {
                pml::log::log(pml::log::Level::kWarning, "pml::restgoose") << "WebsocketClient\tWebsocket closed by server";
...
void WebSocketClientImpl::CloseEvent(mg_connection* pConnection)
{
    ...
    if(auto itError = m_mConnectionError.find(pConnection->id); itError != m_mConnectionError.end())
    {
        ...
        pml::log::log(pml::log::Level::kWarning, "pml::restgoose") << "WebsocketClient\tWebsocket receieved a close event:" << " connection=" << pConnection->id << "\t" << "Error code found = " << nCode;
    }
    else
    {
        pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWebsocket receieved a close event: " << " connection=" << pConnection->id;
    }
...
{
    pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "HandleInitialConnection  connection=" << pConnection->id;
    for(const auto& [theEndpoint, theConnection] : m_mConnection)
    {
        if(theConnection.pConnection == pConnection)
        {
            ...
            {
                pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "WebsocketClient\tConnection with tls";
                mg_tls_opts opts{};
                opts.name = host;
...
{
    pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tStop";
    m_bRun = false;
    if(m_pThread)
    {
        pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tWait on thread";
        m_pThread->join();
        m_pThread = nullptr;
        pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tfinished";
    }
}
...
bool WebSocketClientImpl::DoConnect()
{
    ...
    if(m_qConnection.empty() == false)
    {
        ...
        pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\t" << "Try to connect to " << vSplit[0];

        
        if(auto pConnection = mg_ws_connect(&m_mgr, theEndpoint.Get().c_str(), callback, reinterpret_cast<void*>(this), nullptr); pConnection)
        {
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\t" << " Connection created " << pConnection->id << " to " << theEndpoint;
            m_mConnection.try_emplace(endpoint(vSplit[0]), pConnection);
            return true;
        }
        else
        {
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tFailed to create connection  ";
...
bool WebSocketClientImpl::Connect(const endpoint& theEndpoint)
{
    ...
    {
        pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\t" << "Already connected to " << theEndpoint;
...
void WebSocketClientImpl::CloseConnection(mg_connection* pConnection, bool bTellServer)
{
    ...
    pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tCloseConnection called by client";
...
void WebSocketClientImpl::MarkConnectionConnected(mg_connection* pConnection, bool bConnected, int nCode)
{
    for(auto& [theEndpoint, theConnection] : m_mConnection)
    {
        if(theConnection.pConnection == pConnection)
        {
            ...
            pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tEndpoint found " << foundEndpoint;
            ...
            if(m_pConnectCallback && m_bRun)
            {
                pml::log::log(pml::log::Level::kDebug, "pml::restgoose") << "WebsocketClient\tTell Callback  " << bConnected << " code = " << nCode;
