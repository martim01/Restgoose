    ...
    m_eResponse(eResponse)
{
    ...
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient: convert from json to '" << sData << "'";
...
{
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "Direct Connection";
    ...
    if(mg_url_is_ssl(m_point.second.Get().c_str()))
    {
        ...
        pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient\tConnection is https";
        mg_tls_opts opts{};
        opts.name = host;
    ...
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:SendHeader: " << str;
    mg_send(pConnection, str.c_str(), str.length());
}

void HttpClientImpl::HandleConnectEventToProxy(mg_connection* pConnection)
{
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "Connected to Proxy";
    //if https then do so
    std::string sProto("http://");
...
void HttpClientImpl::HandleReadEvent(mg_connection* pConnection)
{
    if(m_proxy.Get().empty() == false && m_bConnectedViaProxy == false)
    {
        ...
        if(n > 0)
        {
            ...
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:Connected Via Proxy: " << std::string(hm.uri.buf, hm.uri.len);
...
void HttpClientImpl::GetContentHeaders(mg_http_message* pReply)
{
    ...
    if(itType != m_response.mHeaders.end())
    {
        ...
        pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient::Content-Type: " << m_response.contentType;
    ...
    if(itLen != m_response.mHeaders.end())
    {
        try
        {
            ...
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient::Content-Length: " << m_response.nContentLength;

        }
        catch(const std::invalid_argument& e)
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient\tFailed to get content length: " << e.what() << " " << itLen->second.Get();
        }
...
void HttpClientImpl::GetResponseCode(mg_http_message* pReply)
{
    try
    {
        ...
        pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient::Resonse code: " << m_response.nHttpCode;
    }
    catch(const std::invalid_argument& e)
    {
...
{
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:RawReply: " << std::string(pReply->message.buf, pReply->message.len);
    ...
    {   //redirects
        pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:Reply: Redirect";
    ...
    else
    {
        ...
        if(m_response.bBinary == false)
        {
            ...
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:Reply: " << m_response.data;
        }
        ...
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient\tSent binary data but could not open a file to write it to";
        }
        m_eStatus = HttpClientImpl::kComplete;
    }
...
{
    /*
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:RawChunk: " << std::string(pReply->chunk.ptr, pReply->chunk.len);
...
void HttpClientImpl::HandleErrorEvent(const char* error)
{
    ...
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient\tError event: " << m_response.data.Get();
}
...
static void evt_handler(mg_connection* pConnection, int nEvent, void* pEventData)
{
    ...
    {
        pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient:MG_EV_HTTP_CHUNK";
        auto pReply = reinterpret_cast<mg_http_message*>(pEventData);
...
void HttpClientImpl::RunAsync(const std::function<void(const clientResponse&, unsigned int, const std::string& )>& pCallback, unsigned int nRunId, const std::string& sUserData, const std::chrono::milliseconds& connectionTimeout, const std::chrono::milliseconds& processTimeout)
{
    ...
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient::RunAsync: nRunId = " << nRunId << " Endpoint: " << m_point.second;
...
const clientResponse& HttpClientImpl::Run(const std::chrono::milliseconds& connectionTimeout, const std::chrono::milliseconds& processTimeout)
{
    ...
    pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient::Run - connect to " << m_point.second;
    mg_mgr mgr;
    mg_mgr_init(&mgr);
    ...
    {
        pml::log::log(pml::log::Level::kError, "pml::restgoose") << "RestGoose:HttpClient\tCould not create connection";
    ...
    else
    {
        if(m_proxy.Get().empty())
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient::Run - connecting " << m_point.second;
        }
        else
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient::Run - connecting " << m_point.second << " via proxy " << m_proxy;
        }
        DoLoop(mgr);
        ...
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient\tComplete";
        }
        else
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "RestGoose:HttpClient\tTimed out";
            //timed out
...
void HttpClientImpl::DoLoop(mg_mgr& mgr) const
{
    ...
    while(m_eStatus != HttpClientImpl::kRedirecting && m_eStatus != HttpClientImpl::kComplete)
    {
        ...
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClientImpl\tTimeout";
            break;
        }
    }
}
...
bool HttpClientImpl::SendFile(mg_connection* pConnection, const std::filesystem::path& filepath, bool bOpen)
{
    if(bOpen)
    {
        ...
        if(m_ifs.is_open() == false)
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient: Unable to open file " << filepath << " to upload.";
            m_response.nHttpCode = clientResponse::enumError::kErrorFileRead;
            m_eStatus = kComplete;
        }
    }
    if(m_ifs.is_open())
    {
        ...
        {
            pml::log::log(pml::log::Level::kTrace, "pml::restgoose") << "HttpClient: Unable to read file " << filepath << " to upload.";
            m_response.nHttpCode = clientResponse::enumError::kErrorFileRead;
            m_eStatus = kComplete;
        }
    }
