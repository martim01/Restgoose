...
void HttpClientImpl::HandleReadEvent(mg_connection* pConnection)
{
    if(m_proxy.Get().empty() == false && m_bConnectedViaProxy == false)
    {
        ...
        if(n > 0)
        {
            ...
            //send the connection headers
            std::stringstream ss;
            ss << m_point.first.Get() << " /" << vSplit[1] << " HTTP/1.1" << CRLF
               << "Host: " << vSplit[0] << CRLF;
            if(m_contentType.Get().empty() == false)
            {
                ss << "Content-Type: " << m_contentType.Get() << CRLF;
...
void HttpClientImpl::GetContentHeaders(mg_http_message* pReply)
{
    ...
    if(itLen != m_response.mHeaders.end())
    {
        ...
        {
            pmlLog(pml::LOG_WARN, "pml::restgoose") << "RestGoose:HttpClient\tFailed to get content length: " << e.what() << " " << itLen->second.Get();
        }
    }
...
void HttpClientImpl::HandleMessageEvent(mg_http_message* pReply)
{
    ...
    else
    {
        ...
        {
            pmlLog(pml::LOG_WARN, "pml::restgoose") << "RestGoose:HttpClient\tSent binary data but could not open a file to write it to";
        }
        m_eStatus = HttpClientImpl::COMPLETE;
    }
...
void HttpClientImpl::RunAsync(std::function<void(const clientResponse&, unsigned int)> pCallback, unsigned int nRunId, const std::chrono::milliseconds& connectionTimeout, const std::chrono::milliseconds& processTimeout)
{
    ...
    m_nRunId = nRunId;
    pmlLog(pml::LOG_TRACE, "pml::restgoose") << "RestGoose:HttpClient::RunAsync: nRunId = " << nRunId << " Endpoint: " << m_point.second;// << " data: " << m_vPostData.back().filepath;
...
void HttpClientImpl::DoLoop(mg_mgr& mgr)
{
    ...
    while(m_eStatus != HttpClientImpl::REDIRECTING && m_eStatus != HttpClientImpl::COMPLETE)
    {
        ...
        {
            pmlLog(pml::LOG_DEBUG, "pml::restgoose") << "HttpClientImpl\tTimeout";
            break;
        }
    }
}
...
bool HttpClientImpl::SendFile(mg_connection* pConnection, const fileLocation& filepath, bool bOpen)
{
    if(bOpen)
    {
        ...
        if(m_ifs.is_open() == false)
        {
            pmlLog(pml::LOG_ERROR, "pml::restgoose") << "HttpClient: Unable to open file " << filepath << " to upload.";
            m_response.nHttpCode = clientResponse::enumError::ERROR_FILE_READ;
            m_eStatus = COMPLETE;
        }
    }
    if(m_ifs.is_open())
    {
        ...
        {
            pmlLog(pml::LOG_ERROR, "pml::restgoose") << "HttpClient: Unable to read file " << filepath << " to upload.";
            m_response.nHttpCode = clientResponse::enumError::ERROR_FILE_READ;
            m_eStatus = COMPLETE;
        }
    }
